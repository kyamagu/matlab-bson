...
%DOWNLOAD_EJDB Download the latest EJDB package from GitHub.
  libbson = fullfile(root_dir, 'libbson-master');
  if ~exist(libbson, 'dir')
    github_url = 'https://github.com/mongodb/libbson/archive/master.zip';
    zip_file = fullfile(root_dir, 'libbson-master.zip');
  ...
  if ~exist(config.libbson_path, 'file')
    cmd = sprintf('cd %s; ./autogen.sh; make CFLAGS="$CFLAGS -fPIC";', ...
                  libbson);
    disp(cmd);
    system(cmd);
  end
